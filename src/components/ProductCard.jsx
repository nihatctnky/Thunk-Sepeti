...
const ProductCard = ({ item }) => {
    ...
    // prop olarak gelen ürün sepete daha önce eklenmiş mico
    const found = cart?.find((cartItem) => cartItem.productId === item.id);

    const handleAdd = () => {
        found ? dispatch(updateItem(found.id, found.amount + 1)) : dispatch(createItem(item));
    }
    ...
    return (
        <div className="grid grid-cols-[1fr_115px] gap-3 border shadow p-3 rounded-lg hover:bg-red-100 hover:scale-[1.02] cursor-pointer transition duration-300">
            ...
            <div className="relative">
                <img src={item.photo || "https://picsum.photos/id/1015/5173/200"} className="rounded-md object-cover size-full" />

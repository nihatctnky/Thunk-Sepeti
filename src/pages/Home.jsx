...
const Home = () => {
    ...
    return (
        <div className="container">
            <h1 className="font-semibold text-lg md:text-xl">Yakınızdaki Restoranlar</h1>
            {isLoading ?
                (<Loader />) :
                error ?
                    (<Error info={error} retry={retry} />) :
                    (<div className="grid grid-cols-1 md:grid-cols-2
                lg:grid-cols-3 gap-5 mt-5">{restaurants.map((item) => (<RestaurantCard key={item.id} data={item} />))}
                    </div>
                    )}
        </div>
    )
